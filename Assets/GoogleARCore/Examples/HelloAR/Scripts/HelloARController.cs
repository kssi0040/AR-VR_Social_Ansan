        ...
        public void Update()
        {
            ...
            */
            ...
            else if (ROUTE.SELECT_FOOD == eRoute)
            {
                ...
                for(int i = 0; i < 4; ++i)
                {
                    if(XML_Reader.Instance.scenarioToDict.BranchDictionary[m_StagePlay.sceneLoader.currentStage][XML_Reader.Instance.scenarioToDict.QuarterEventDictionary[XML_Reader.Instance.scenarioToDict.StageSetDictionary[m_StagePlay.sceneLoader.currentStage].PageList[m_StagePlay.Index].EventID].indexList.Group][i] == true)
                    {
                        {
                            FoodObject[i].transform.GetChild(0).GetComponent<MeshRenderer>().material.color = Color.green;
                        }
                    }                  
                }
                for(int k=0;k<FoodObject.Length;k++)
                {
                    Debug.Log(FoodObject[k].name);
                }
            ...
            if (Input.touchCount == 1)
            {
                ...
                if (true == bCheck)
                {
                    ...
                    if (11 == iLayer)                                                                                               // curry 선택시에
                    {
                        for (int i = 0; i < 4; ++i)
                        {
                            if(FoodObject[i].name==hit.collider.gameObject.name)
                            {
                                temp = i;
                            }
                        }
                        if (XML_Reader.Instance.scenarioToDict.BranchDictionary[m_StagePlay.sceneLoader.currentStage][XML_Reader.Instance.scenarioToDict.QuarterEventDictionary[XML_Reader.Instance.scenarioToDict.StageSetDictionary[m_StagePlay.sceneLoader.currentStage].PageList[m_StagePlay.Index].EventID].indexList.Group][temp] == false)
                        {
                            CurrySelect();
                        }
                        //CurrySelect();
