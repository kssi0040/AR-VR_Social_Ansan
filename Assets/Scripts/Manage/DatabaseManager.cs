...
public class DatabaseManager : MonoBehaviour
{
    ...
    //============================================== 기록 부분 ===============================================//
    //Record....
    public void RecordGetStamp(string _strAppID, string _strUserID, string _strBadgeType, string _strEduType)
    {
        StartCoroutine(RecordGetStameInfo(_strAppID, _strUserID, _strBadgeType, _strEduType));
    }
    IEnumerator RecordGetStameInfo(string _strAppID, string _strUserID, string _strBadgeType, string _strEduType)
    {
        WWWForm form = new WWWForm();
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("http://eduarvr.dlinkddns.com//pages/unity_php/sticker_check.php", form))
        {
            ...
            else
            {
                ...
                string strHandlerText = www.downloadHandler.text;
                string resultText = strHandlerText.Trim();
                GameObject.Find("CollectionSystem").SendMessage("RecordCatchStampInfo", resultText);
            }
        }
    }

    public void RecordTimeAtkRank(string _strAppID, string _strUserID, string _strBadgeType, string _strEduType)
    {
        StartCoroutine(RecordTimeAtkRankProcess(_strAppID, _strUserID, _strBadgeType, _strEduType));
    }
    IEnumerator RecordTimeAtkRankProcess(string _strAppID, string _strUserID, string _strBadgeType, string _strEduType)
    {
        WWWForm form = new WWWForm();
        form.AddField("appID", _strAppID);
        form.AddField("userID", _strUserID);
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("http://eduarvr.dlinkddns.com/pages/unity_php/time_report_ranking.php", form))
        {
            yield return www.SendWebRequest();
            if (www.isNetworkError || www.isHttpError)
            {
            ...
            else
            {
                ...
                // 수정 예정...
                GameObject.Find("CollectionSystem").SendMessage("RecordCatchRank", resultText);                                
            }
        }
    }
